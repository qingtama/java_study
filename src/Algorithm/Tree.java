...
public class Tree<AnyType extends Comparable<? super AnyType>>
{
     ...
     * 非递归
     */
    public void preOrder2(BinaryNode<AnyType> node){
        Stack<BinaryNode> stack = new Stack<>();
        while (node != null || !stack.empty()) {
            if (node != null) {
                System.out.print(node.element + "   ");
                ...
                node = stack.pop().right;        //回溯至父亲结点，访问右子树
            }
        }
    }
     ...
     * 非递归
     */
    public void midOrder2(BinaryNode<AnyType> node)
    {
        ...
        while (node != null || !s.empty()) {
            if (node != null) {
                ...
                node = s.pop();
                System.out.print(node.element + "   ");  //访问完左子树后才访问根结点
    ...
    public void posOrder2(BinaryNode<AnyType> node)
    {
        if (node == null) return;
        Stack<BinaryNode> s = new Stack<>();
